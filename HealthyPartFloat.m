...
for k=1:length(trials)
    for i=1:length(PosIC_left.(trials{k}))-1
        for j=1:length(sensors)
            ...
            %stridelength
            S6_FLOAT.(trials{k}).GaitCycles.(numbers{i}).Kin.StrideLength.(sensors{j})=sqrt((S6_FLOAT.(trials{k}).GaitCycles.(numbers{i}).Kin.(sensors{j})(end,2)-S6_FLOAT.(trials{k}).GaitCycles.(numbers{i}).Kin.(sensors{j})(1,2))^2+(S6_FLOAT.(trials{k}).GaitCycles.(numbers{i}).Kin.(sensors{j})(end,1)-S6_FLOAT.(trials{k}).GaitCycles.(numbers{i}).Kin.(sensors{j})(1,1))^2);
...
for k=1:length(trials)
    ...
    for i=1:length(PosIC_left.(trials{k}))-1
        ...
        for j=1:length(sensors)
            ...
            % max knee height
            S6_FLOAT.(trials{k}).GaitCycles.(numbers{i}).Kin.MaxHeelClearance.(sensors{j})=max(S6_FLOAT.(trials{k}).GaitCycles.(numbers{i}).Kin.(sensors{j})(:,3));
    ...
    for k=1:length(trials)
        ...
        for j=1:length(PosIC_left.(trials{k}))-1
            ...
            for i=1:size(S6_FLOAT.(trials{k}).GaitCycles.(numbers{j}).Kin.(sensors{n,1}),1)
                ...
                TOE_points(i,:)=[S6_FLOAT.(trials{k}).GaitCycles.(numbers{j}).Kin.(sensors{n,1})(i,1) S6_FLOAT.(trials{k}).GaitCycles.(numbers{j}).Kin.(sensors{n,1})(i,2) S6_FLOAT.(trials{k}).GaitCycles.(numbers{j}).Kin.(sensors{n,1})(i,3)];
                ANK_points(i,:)=[S6_FLOAT.(trials{k}).GaitCycles.(numbers{j}).Kin.(sensors{n,2})(i,1) S6_FLOAT.(trials{k}).GaitCycles.(numbers{j}).Kin.(sensors{n,2})(i,2) S6_FLOAT.(trials{k}).GaitCycles.(numbers{j}).Kin.(sensors{n,2})(i,3)];
                KNE_points(i,:)=[S6_FLOAT.(trials{k}).GaitCycles.(numbers{j}).Kin.(sensors{n,3})(i,1) S6_FLOAT.(trials{k}).GaitCycles.(numbers{j}).Kin.(sensors{n,3})(i,2) S6_FLOAT.(trials{k}).GaitCycles.(numbers{j}).Kin.(sensors{n,3})(i,3)];
                HIP_points(i,:)=[S6_FLOAT.(trials{k}).GaitCycles.(numbers{j}).Kin.(sensors{n,4})(i,1) S6_FLOAT.(trials{k}).GaitCycles.(numbers{j}).Kin.(sensors{n,4})(i,2) S6_FLOAT.(trials{k}).GaitCycles.(numbers{j}).Kin.(sensors{n,4})(i,3)];
                ...
                Vertical = [0 0 1];
                Horizontal = S6_FLOAT.(trials{k}).GaitCycles.(numbers{j}).Kin.(sensors{n,1})(end,:)-S6_FLOAT.(trials{k}).GaitCycles.(numbers{j}).Kin.(sensors{n,1})(1,:);
...
Features.MaxToeClereanceL(j+l,1)=S6_FLOAT.(trials{k}).GaitCycles.(numbers{j}).Kin.StepHeight.(all_sensors{2});
Features.MaxToeClereanceR(j+l,1)=S6_FLOAT.(trials{k}).GaitCycles.(numbers{j}).Kin.StepHeight.(all_sensors{6});
...
Features.ElevationKneeRmin(j+l,1)=min(S6_FLOAT.(trials{k}).GaitCycles.(numbers{j}).Kin.RightAngles.KneeElevAngle);
...
Features.ElevationAnkleLmax(j+l,1)=max(S6_FLOAT.(trials{k}).GaitCycles.(numbers{j}).Kin.LeftAngles.AnkleElevAngle);
Features.ElevationAnkleRmax(j+l,1)=max(S6_FLOAT.(trials{k}).GaitCycles.(numbers{j}).Kin.RightAngles.AnkleElevAngle);

Features.ElevationAnkleLmin(j+l,1)=min(S6_FLOAT.(trials{k}).GaitCycles.(numbers{j}).Kin.LeftAngles.AnkleElevAngle);
Features.ElevationAnkleRmin(j+l,1)=min(S6_FLOAT.(trials{k}).GaitCycles.(numbers{j}).Kin.RightAngles.AnkleElevAngle);
...
Features.MaxHeelClearanceL(j+l,1)=S6_FLOAT.(trials{k}).GaitCycles.(numbers{j}).Kin.MaxHeelClearance.(all_sensors{1});
Features.MaxHeelClearanceR(j+l,1)=S6_FLOAT.(trials{k}).GaitCycles.(numbers{j}).Kin.MaxHeelClearance.(all_sensors{5});

Features.StrideLengthL(j+l,1)=S6_FLOAT.(trials{k}).GaitCycles.(numbers{j}).Kin.StrideLength.(all_sensors{2});
Features.StrideLengthR(j+l,1)=S6_FLOAT.(trials{k}).GaitCycles.(numbers{j}).Kin.StrideLength.(all_sensors{6});
...
for k=1:length(trials)
     for i=1:4
         subplot(3,4,4*(k-1)+i)
         plot([0:(length(S6_FLOAT.(trials{k}).Raw.EMG.(EMGSensors{i}))-1)]/(S6_FLOAT.(TrialName).fsEMG),S6_FLOAT.(trials{k}).Raw.EMG.(EMGSensors{i}));
...
for k=1:length(trials)
    for i=1:length(fieldnames(S6_FLOAT.(trials{k}).GaitCycles))
         for j=2%[2,3]
            ...
            StdNoise{k,1}(i)=min([std(S6_FLOAT.(trials{k}).GaitCycles.(numbers{i}).EMG.Raw.(EMGSensors{j})(750:1250)),0.30*max(S6_FLOAT.(trials{k}).GaitCycles.(numbers{i}).EMG.Filtered4.(EMGSensors{j}))]);
            indeces=find(S6_FLOAT.(trials{k}).GaitCycles.(numbers{i}).EMG.Filtered4.(EMGSensors{j})>=(2*StdNoise{k,1}(i)));
...
for k=1:length(trials)
    for i=1:length(fieldnames(S6_FLOAT.(trials{k}).GaitCycles))
         for j=3%[2,3]
            %Assumption#1.1: for LTA between 750 and 1250 it is always noise
            %Assumption#1.2: we put a higher limit for 2*std (threshold
            %cannot be more than 40% the maximum amplitude of the filtered
            %signal)=>2std<=0.4/2*max=0.2*max
            StdNoise{k,2}(i)=min([std(S6_FLOAT.(trials{k}).GaitCycles.(numbers{i}).EMG.Raw.(EMGSensors{j})(750:1250)),0.2*max(S6_FLOAT.(trials{k}).GaitCycles.(numbers{i}).EMG.Filtered4.(EMGSensors{j}))]);
            indeces=find(S6_FLOAT.(trials{k}).GaitCycles.(numbers{i}).EMG.Filtered4.(EMGSensors{j})>=(2*StdNoise{k,2}(i)));
            %Assumption#2: minimum burst length is 350
            Bursts.Position.(trials{k}).GaitCycles.(numbers{i}).(EMGSensors{j})=[];
            for l=1:(length(indeces)-1)
               if (indeces(l+1)-indeces(l)>350)  
...
for i=1:length(fieldnames(Bursts.Position.(trials{k}).GaitCycles))
    ...
    asse=[1:length(S6_FLOAT.(trials{k}).GaitCycles.(numbers{i}).EMG.Raw.(EMGSensors{l}))]/1000;   
    subplot(NoRows,2,2*(i-1)+1)
    plot(asse,S6_FLOAT.(trials{k}).GaitCycles.(numbers{i}).EMG.Raw.(EMGSensors{l}))
    ylabel(sprintf('Gait cycle %s',numbers{i}))
    hold on
    subplot(NoRows,2,2*(i-1)+1)
    %YRect=S6_FLOAT.(trials{k}).GaitCycles.(numbers{i}).EMG.Raw.(EMGSensors{l})(750);
    YRect=mean(S6_FLOAT.(trials{k}).GaitCycles.(numbers{i}).EMG.Raw.(EMGSensors{l})(750:1250))-StdNoise{k,l-1}(i);
    rectangle('Position',[0.75 YRect 0.5 2*StdNoise{k,l-1}(i)],'EdgeColor','r')
    subplot(NoRows,2,2*i)
    plot(asse,S6_FLOAT.(trials{k}).GaitCycles.(numbers{i}).EMG.Filtered4.(EMGSensors{l}))
    hold on
    subplot(NoRows,2,2*i)
    plot(asse,2* StdNoise{k,l-1}(i)*ones(length(asse),1),':r')
    subplot(NoRows,2,2*i)
    plot(z1/1000,S6_FLOAT.(trials{k}).GaitCycles.(numbers{i}).EMG.Filtered4.(EMGSensors{l})(z1),'xr')
    subplot(NoRows,2,2*i)
    plot(z2/1000,S6_FLOAT.(trials{k}).GaitCycles.(numbers{i}).EMG.Filtered4.(EMGSensors{l})(z2),'xr')
