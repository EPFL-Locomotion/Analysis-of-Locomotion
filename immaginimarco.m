   ...
   if isstruct(S6_NO_FLOAT)
    for Sensoridx=1:length(EMGSensors)
        ...
        if isstruct(S6_NO_FLOAT.(TrialName).Raw.EMG)
            ...
            [lp1,lp2]=butter(8,10/(S6_NO_FLOAT.(TrialName).fsEMG/2),'low'); 
...
for i=1:length(S6_NO_FLOAT.T_01.Raw.Kin.LTOE)
    ...
    % plot3([S6_NO_FLOAT.T_01.Raw.Kin.LTOE(i,1), S6_NO_FLOAT.T_01.Raw.Kin.LANK(i,1)], [S6_NO_FLOAT.T_01.Raw.Kin.LTOE(i,2), S6_NO_FLOAT.T_01.Raw.Kin.LANK(i,2)], [S6_NO_FLOAT.T_01.Raw.Kin.LTOE(i,3), S6_NO_FLOAT.T_01.Raw.Kin.LANK(i,3)]);
...
for k=1:length(trials)
    for i=1:length(fieldnames(S6_NO_FLOAT.(trials{k}).GaitCycles))
         for j=3%[2,3]
            ...
            %clear StdNoise;
            clear indeces;
            StdNoise{k,1}(i)=min([std(S6_NO_FLOAT.(trials{k}).GaitCycles.(numbers{i}).EMG.Raw.(EMGSensors{j})(350:600)),0.0375/2]);
...
for i=1:3
    asse=[1:length(S6_NO_FLOAT.(trials{2}).GaitCycles.(numbers{i}).EMG.Raw.(EMGSensors{2}))]/1000;   
    subplot(3,2,2*(i-1)+1)
    plot(asse,S6_NO_FLOAT.(trials{2}).GaitCycles.(numbers{i}).EMG.Raw.(EMGSensors{2}))
    subplot(3,2,2*i)
    plot(asse,S6_NO_FLOAT.(trials{2}).GaitCycles.(numbers{i}).EMG.Filtered4.(EMGSensors{2}))
    hold on
    subplot(3,2,2*i)
    plot(asse,2*StdNoise*ones(length(asse),1),':c')
end
%%
